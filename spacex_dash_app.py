...
#Creation of dataset ready for pie chart
a = spacex_df.groupby(['Launch Site'], as_index=False).count()
a = a[['Launch Site', 'class']]
b = spacex_df.groupby(['Launch Site'], as_index=False).sum()
b = b[['Launch Site', 'class']]
c = a[['class']] - b[['class']]
d = a[['Launch Site']]
d['Success'] = b[['class']]
d['Unsuccess'] = c[['class']]
...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        fig = px.pie(filtered_df, values='Launch Site', names='Launch Site', title = entered_site)
        fig.show()
        return fig        
# TASK 4:
